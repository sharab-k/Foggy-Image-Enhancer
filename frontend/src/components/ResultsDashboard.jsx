      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <span className={`scene-badge ${original?.day_night === 'Night' ? 'badge-night' : 'badge-day'}`}>
            {original?.day_night ? `${original.day_night} Mode` : 'Unknown Mode'}
          </span>
        </div>
      ...
      <div className="result-grid">
        ...
        <div className="result-card">
          ...
          <div className="result-stats">
            ...
            <span>Scene: <span className={`stat-value ${active?.day_night === 'Night' ? 'text-night' : 'text-day'}`}>{active?.day_night || 'Unknown'}</span> ({active?.brightness !== undefined ? active.brightness.toFixed(0) : '?'})</span>
          </div>
        </div>
      </div>
